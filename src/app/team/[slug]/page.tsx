...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = seo?.meta_description || bio.replace(/<[^>]*>/g, '').slice(0, 160) || `${name} — Team at DeAI Summit 2026`;
  const canonical = seo?.canonical_url || `${BASE_URL}/team/${member.person_slug}`;
...
export default async function TeamMemberDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schema = generatePersonSchema(member, BASE_URL, 'team');
