...
export async function generateMetadata(): Promise<Metadata> {
  const cmsPage = await prefetchCMSPage('terms-and-conditions');
  return generatePageMetadata(cmsPage?.seo || null, 'terms', BASE_URL);
}

export default async function TermsPage() {
  const [cmsPage, apiNav, socials] = await Promise.all([
    prefetchCMSPage('terms-and-conditions'),
    prefetchNavigation(),
    prefetchSocials(),
  ]);
  ...
  const title = (firstBlock?.title as string) || 'General Terms & Conditions';
